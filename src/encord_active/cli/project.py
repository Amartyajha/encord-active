...
@project_cli.command(
    name="download-data",
    short_help="Download all data locally for improved responsiveness.",
)
@ensure_project()
def download_data(
    ...
    with PrismaConnection(project_file_structure) as conn:
        ...
        for label in tqdm(
            project_file_structure.iter_labels(cache_db=conn),
            desc="Downloading project data locally",
        ):
            ...
            for db in label.iter_data_unit():
                ...
                file_path = (
                    project_file_structure.local_data_store / db.du_hash
                ).with_suffix(Path(du["data_title"]).suffix)
                data_uri = file_path_to_url(
                    file_path, project_dir=project_file_structure.project_dir
                )
                download_file(
                    db.signed_url,
