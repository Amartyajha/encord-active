...
def get_data_hashes_from_project(
    project_fs: ProjectFileStructure, subset_size: int = None
) -> list[tuple[str, str]]:
...
def get_data_from_data_hashes(
    ...
    image_urls, class_labels = zip(
        *(
            get_data_sample(project_fs, data_hash, class_name)
...
def get_data_sample(
    ...
    # get image path
    image_url = next(
        du_struct.signed_url for du_struct in lr_struct.iter_data_unit(du_hash)
    )
...
def get_classification_label(label_row, du_hash: str, class_name: str):
    ...
    filtered_class = [
        _class
        for _class in data_unit["labels"]["classifications"]
...
def get_n_best_ranked_data_samples(
    ...
    if filter_by_data_hashes is not None:
        str_data_hashes = tuple(
            f"{label_hash}_{du_hash}" for label_hash, du_hash in filter_by_data_hashes
        )
        acq_func_results = acq_func_results[
            acq_func_results["identifier"].str.startswith(str_data_hashes, na=False)
        ]

    if exclude_data_hashes is not None:
        str_data_hashes = tuple(
            f"{label_hash}_{du_hash}" for label_hash, du_hash in exclude_data_hashes
        )
        acq_func_results = acq_func_results[
            ~acq_func_results["identifier"].str.startswith(str_data_hashes, na=False)
        ]
    ...
    if (
        rank_by == "asc"
    ):  # get the first n data samples if they were sorted by ascending score order
        best_n = acq_func_results[["identifier", "score"]].nsmallest(
            n, "score", keep="first"
        )
    elif (
        rank_by == "desc"
    ):  # get the first n data samples if they were sorted by descending score order
        best_n = acq_func_results[["identifier", "score"]].nlargest(
            n, "score", keep="first"
        )
    else:
        raise ValueError
    ...
    data_hashes = tuple(
        get_data_hash_from_identifier(identifier) for identifier in identifiers
    )
