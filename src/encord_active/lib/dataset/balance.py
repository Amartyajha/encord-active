...
def balance_dataframe(
    selected_metrics: List[MetricData], partition_sizes: Dict[str, int], seed: int
) -> pd.DataFrame:
    """
    ...
    for i, m in enumerate(selected_metrics):
        ...
        merged_df_list.append(
            df[["identifier", "score"]].rename(columns={"score": m.name})
        )
    ...
    merged_df["partition"] = ""
    for partition_name, partition_size in [(k, v) for k, v in partition_sizes.items()][
        :-1
    ]:
        n_partition = int(np.floor(n_samples * partition_size / 100))
        partition_df = selection_df.sample(
            n=n_partition, replace=False, random_state=seed
        )
        # Remove samples from selection_df
        selection_df = selection_df[
            ~selection_df["identifier"].isin(partition_df["identifier"])
        ]
    ...
    merged_df.loc[merged_df["partition"] == "", "partition"] = list(
        partition_sizes.keys()
    )[-1]
    return merged_df


def get_partitions_zip(
    ...
    with ZipFile(zip_io, mode="w") as zf:
        ...
        for partition_name, partition in tqdm(
            partition_dict.items(), desc="Generating COCO files"
        ):
            coco_json = generate_coco_file(
                partition,
                project_file_structure.project_dir,
                project_file_structure.ontology,
            )
            with zf.open(
                partition_name.replace(" ", "_").lower() + ".json", "w"
            ) as zip_file:
