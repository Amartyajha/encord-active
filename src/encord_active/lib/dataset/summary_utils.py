...
def get_all_image_sizes(project_file_structure: ProjectFileStructure) -> np.ndarray:
    ...
    with PrismaConnection(project_file_structure) as conn:
        ...
        image_sizes = [
            [data_unit["width"], data_unit["height"]] for data_unit in results
        ]
...
def get_all_annotation_numbers(
    ...
    project_ontology = json.loads(
        project_file_structure.ontology.read_text(encoding="utf-8")
    )
                ...
                for data_unit in label_row_meta["data_units"].values():
                    ...
                    for object_ in data_unit["labels"].get("objects", []):
                        if object_["name"] not in labels.objects:
                            print(
                                f'Object name "{object_["name"]}" is not exist in project ontology'
                            )
                    ...
                    for classification in data_unit["labels"].get(
                        ...
                        if (
                            classificationHash
                            not in label_row_meta["classification_answers"]
                        ):
                            logging.warning(
                        ...
                        classification_answer_item = label_row_meta[
                            "classification_answers"
                        ][classificationHash]["classifications"][0]
                        classification_question_name = classification_answer_item[
                            "name"
                        ]
                        if classification_question_name in labels.classifications:
                            if isinstance(classification_answer_item["answers"], list):
                                for answer_item in classification_answer_item[
                                    "answers"
                                ]:
                                    if (
                                        answer_item["name"]
                                        in labels.classifications[
                                ...
                                labels.classifications[
                                    classification_question_name
                                ].setdefault(classification_answer_item["answers"], 0)
...
def get_metric_summary(metrics: list[MetricData]) -> MetricsSeverity:
    ...
    for metric in metrics:
        ...
        metric_severity.metrics[metric.name] = MetricOutlierInfo(
            metric=metric,
            df=DataFrame[MetricWithDistanceSchema](df),
