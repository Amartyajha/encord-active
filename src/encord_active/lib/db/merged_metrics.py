...
def build_merged_metrics(metrics_path: Path) -> pd.DataFrame:
    ...
    for index in metrics_path.glob("*.csv"):
        ...
        if len(metric_scores["identifier"][0].split("_")) == 3:
            ...
            main_df_images = pd.merge(
                main_df_images, columns_to_merge, how="outer", on="identifier"
            )
            ...
            if (
                ...
                main_df_image_quality.rename(
                    columns={"score": f"{meta.title}"}, inplace=True
                )
                continue

            # Object-level index
            if main_df_objects.shape[0] == 0:
                columns_to_merge = metric_scores[
                    ["identifier", "url", "score", "object_class", "annotator"]
                ]
            else:
                columns_to_merge = metric_scores[["identifier", "score"]]
            main_df_objects = pd.merge(
                main_df_objects, columns_to_merge, how="outer", on="identifier"
            )
...
def ensure_initialised_merged_metrics(project_file_structure: ProjectFileStructure):
    try:
        with DBConnection(project_file_structure) as conn:
            ...
            missing_columns = MANDATORY_COLUMNS - MANDATORY_COLUMNS.intersection(
                set(columns["name"])
            )
            if missing_columns:
                prev = MergedMetrics(conn)._unsafe_all(False, None)
                new_merged_metrics = build_merged_metrics(
                    project_file_structure.metrics
                )
                new_merged_metrics.drop("tags", axis=1, inplace=True)
                new_merged_metrics = new_merged_metrics.join(
                    prev["tags"], on="identifier", how="left"
                )
...
class MergedMetrics(object):
    ...
    def get_row(self, id: str):
        r = pd.read_sql(
            f"SELECT * FROM {TABLE_NAME} where IDENTIFIER = '{id}'", self.connection
        )
    ...
    def update_tags(self, id: str, tags: List[Tag]):
        self.connection.execute(
            f"UPDATE {TABLE_NAME} SET tags = ? WHERE IDENTIFIER = ?",
            (marshall_tags(tags), id),
        )
    ...
    def replace_identifiers(self, mappings: dict[str, str]):
        ...
        df = pd.read_sql(
            f"SELECT * FROM {TABLE_NAME}", self.connection, index_col="identifier"
        )
