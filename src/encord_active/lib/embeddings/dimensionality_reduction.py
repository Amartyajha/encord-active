...
warnings.filterwarnings(
    "ignore", "n_neighbors is larger than the dataset size", category=UserWarning
)
...
def generate_2d_embedding_data(
    ...
    embeddings_2d_collection: dict[str, list] = {
        "identifier": [],
        "x": [],
        "y": [],
        "label": [],
    ...
    for label_embedding, emb_2d in zip(label_embeddings, embeddings_2d):
        identifier_no_label = f'{label_embedding["label_row"]}_{label_embedding["data_unit"]}_{int(label_embedding["frame"]):05d}'
        if embedding_type == EmbeddingType.IMAGE:
            ...
            embeddings_2d_collection["identifier"].append(
                f'{identifier_no_label}_{label_embedding["labelHash"]}'
            )
    ...
    target_path = project_file_structure.get_embeddings_file(
...
def get_2d_embedding_data(
    ...
    embedding_file_path = project_file_structure.get_embeddings_file(
        embedding_type, reduced=True
    )
