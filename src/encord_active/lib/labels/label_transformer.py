...
class LabelTransformerWrapper:
    ...
    def _get_object_by_name(
        self, name: str, shape: Shape, create_when_missing=False
    ) -> Object:
        obj = next(
            (
                o
                for o in self.ontology_structure.objects
                if o.name == name and o.shape == shape
            ),
            None,
        )
        if obj is None:
...
This will make a (potentially new) classification question `{LabelTransformerWrapper.ATTRIBUTE_NAME} and add the option `{label.class_}`."""
        ...
        attribute = next(
            (
                ...
                if a.name == LabelTransformerWrapper.ATTRIBUTE_NAME
                and isinstance(a, RadioAttribute)
            ),
            None,
        )
        if attribute is None:
            attribute = classification.add_attribute(
                RadioAttribute, LabelTransformerWrapper.ATTRIBUTE_NAME
            )
    ...
    def _add_classification_label(
        self, label: ClassificationLabel, label_row: dict, data_unit: dict
    ):
        clf, attr, opt = self._get_radio_classification_by_name(
            label, create_when_missing=True
        )
        clf_obj, clf_answer = make_classification_dict_and_answer_dict(clf, attr, opt)
        data_unit.setdefault("labels", {}).setdefault("classifications", []).append(
            clf_obj
        )
        label_row.setdefault("classification_answers", {})[
            clf_obj["classificationHash"]
        ] = clf_answer

    def _add_bounding_box_label(self, label: BoundingBoxLabel, data_unit: dict):
        shape = (
            Shape.BOUNDING_BOX
            if label.bounding_box.theta == 0.0
            else Shape.ROTATABLE_BOUNDING_BOX
        )

        ont_obj = self._get_object_by_name(
            label.class_, shape=shape, create_when_missing=True
        )
    ...
    def _add_polygon_label(self, label: PolygonLabel, data_unit: dict):
        ont_obj = self._get_object_by_name(
            label.class_, shape=Shape.POLYGON, create_when_missing=True
        )
    ...
    def add_labels(self, label_paths: List[Path], data_paths: List[Path]):
        ...
        for data_label in self.label_transformer.from_custom_labels(
            label_paths, data_paths
        ):
            du = self.file_to_data_unit_map.get(
                data_label.abs_data_path.expanduser().resolve(), None
            )
            if du is None:
                continue
