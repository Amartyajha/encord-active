...
def fetch_metrics_meta(project_file_structure: ProjectFileStructure):
    ...
    if not meta_file.is_file():
        ...
        for metric_title in builtin_metrics:
            if (
                metric_title in metrics_meta
                and not Path(metrics_meta[metric_title]["location"]).exists()
            ):
                metrics_meta[metric_title] = builtin_metrics[metric_title]

    # Update refreshed metric locations
    update_metrics_meta(project_file_structure, metrics_meta)
    return metrics_meta


def update_metrics_meta(
    project_file_structure: ProjectFileStructure, updated_meta: dict
):
