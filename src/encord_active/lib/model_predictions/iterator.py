...
class PredictionIterator(Iterator):
    def __init__(self, cache_dir: Path, subset_size: Optional[int] = None, **kwargs):
        ...
        with class_idx_file.open("r", encoding="utf-8") as f:
            class_idx: Dict[ClassID, dict] = {
                ClassID(k): v for k, v in json.load(f).items()
            }

        object_lookup = {
            obj.feature_node_hash: obj for obj in self.project.ontology.objects
        }
        classification_lookup = {
            hashes: items
            for hashes, items in iterate_classification_attribute_options(
    ...
    def get_image(
        self, pred: Series, cache_db: "prisma.Prisma"
    ) -> Optional[Image.Image]:
        label_row_structure = self.project.file_structure.label_row_structure(
            pred["label_hash"]
        )
    ...
    def get_encord_object(
        self, pred: Series, width: int, height: int, ontology_object: Object
    ):
        if ontology_object.shape == Shape.BOUNDING_BOX:
            ...
            object_data = {
                str(i): {"x": round(c[0] / width, 4), "y": round(c[1] / height, 4)}
    ...
    def get_encord_classification(
        self, pred: Series, ontology_classification: ClassificationAttributeOption
    ):
                ...
                for frame, fr_preds in lh_group.groupby("frame"):
                    ...
                    for _, prediction in fr_preds.iterrows():
                        ...
                        if class_id in self.ontology_objects:
                            objects.append(
                                self.get_encord_object(
                                    prediction,
                                    width,
                                    height,
                                    self.ontology_objects[class_id],
                                )
                            )
                        elif class_id in self.ontology_classifications:
                            classifications.append(
                                asdict(
                                    self.get_encord_classification(
                                        prediction,
                                        self.ontology_classifications[class_id],
                                    )
                                )
                            )
                        else:
                            logger.error(
                                "The prediction is not in the ontology objects or classifications"
                            )
                    ...
                    if image is None:
                        logger.error(
                            f"Failed to open Image at frame: {self.du_hash}/{fr_preds.iloc[0]}"
                        )
                    yield du, image
                    pbar.update(1)
